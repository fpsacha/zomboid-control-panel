...
function generateStartupBatch(options) {
  ...
  if (zomboidDataPath) {
    gameArgs.push(`-cachedir="${zomboidDataPath}"`);
  }

  if (adminPassword) {
    gameArgs.push(`-adminpassword ${adminPassword}`);
  }
      ...
      try {
        ...
        if (isRunning) {
          ...
          for (let i = 0; i < rconDelays.length; i++) {
            ...
            logger.info(`RCON startup: waiting ${delaySeconds}s before attempt ${i + 1}/${rconDelays.length}...`);
            await new Promise(resolve => setTimeout(resolve, rconDelays[i]));
            ...
            try {
              logger.info(`RCON startup: attempting connection ${i + 1}/${rconDelays.length}...`);
              ...
              if (rconService.connected) {
                logger.info('RCON connected after server startup');
                rconConnected = true;
                break;
              } else {
                logger.info(`RCON startup attempt ${i + 1} - not connected yet`);
              }
            } catch (e) {
              logger.info(`RCON startup connection attempt ${i + 1} failed: ${e.message}`);
          ...
          if (rconConnected) {
            logger.info('RCON startup sequence completed - connected');
          } else {
            logger.warn('RCON startup sequence completed - NOT connected (auto-reconnect will keep trying every 30s)');
          }
          ...
          logger.warn('Server start polling timed out');
        }
      } catch (err) {
        ...
        logger.error(`Server status poll failed: ${err.message}`);
      }
    }, 1000);
...
router.post('/force-stop', async (req, res) => {
  try {
    ...
    logger.error(`Failed to force stop server: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
const FALLBACK_BRANCHES = [
  { name: 'public', description: 'Default stable branch' },
  { name: 'unstable', description: 'Build 42 beta' },
  { name: 'b42mp', description: 'Build 42 multiplayer beta' },
  { name: 'iwbums', description: 'I Will Backup My Save' },
...
router.get('/branches', async (req, res) => {
  try {
    ...
    logger.warn(`Failed to fetch Steam branches: ${error.message}`);
    res.json({ 
      branches: FALLBACK_BRANCHES,
      source: 'fallback',
...
function parseSteamBranches(output) {
  ...
  try {
    ...
    logger.warn(`Failed to parse Steam branches: ${err.message}`);
  }
...
router.post('/install', async (req, res) => {
  try {
    ...
    logger.info(`Starting PZ server installation to ${installPath} (branch: ${selectedBranch})`);
    ...
    steamcmd.stderr.on('data', (data) => {
      ...
      for (const line of lines) {
        if (line.trim()) {
          ...
          logger.warn(`SteamCMD stderr: ${line}`);
        }
      }
    });
    ...
    steamcmd.on('close', async (code) => {
      ...
      if (stdoutBuffer.trim()) {
        ...
        logger.info(`SteamCMD: ${stdoutBuffer.trim()}`);
      }
      if (stderrBuffer.trim()) {
        io.emit('install:log', { type: 'stderr', text: stderrBuffer.trim() });
        logger.warn(`SteamCMD stderr: ${stderrBuffer.trim()}`);
      }
      
      if (code === 0) {
        logger.info('PZ server installation completed successfully');
        ...
        if (zomboidDataPath) {
          ...
          // Use default user profile path
          const userProfile = process.env.USERPROFILE || '';
          zomboidPath = path.join(userProfile, 'Zomboid');
        ...
        try {
          ...
          logger.info(`Created custom startup batch: ${batchPath}`);
          io.emit('install:log', { type: 'stdout', text: `Created custom startup script: StartServer_${serverName}.bat` });
        } catch (batchError) {
          logger.warn(`Failed to create startup batch: ${batchError.message}`);
        }
    ...
    steamcmd.on('error', (error) => {
      ...
      logger.error(`SteamCMD error: ${error.message}`);
      io.emit('install:complete', { 
        success: false, 
...
router.post('/quick-setup', async (req, res) => {
  try {
    ...
    logger.info(`Quick setup: Creating server config for ${serverName} using files from ${installPath}`);
...
router.post('/configure-rcon', async (req, res) => {
  try {
    ...
    logger.info(`RCON configured in ${iniPath}`);
    res.json({ 
      success: true, 
    ...
    logger.error(`Failed to configure RCON: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/configure-network', async (req, res) => {
  try {
    ...
    logger.info(`Network settings configured in ${iniPath}: port=${serverPort}, UPnP=${upnpValue}`);
    res.json({ 
      success: true, 
    ...
    logger.error(`Failed to configure network settings: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/alarm', async (req, res) => {
  try {
    ...
    logger.error(`Failed to trigger alarm: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/removezombies', async (req, res) => {
  try {
    ...
    logger.error(`Failed to remove zombies: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/reloadlua', async (req, res) => {
  try {
    ...
    logger.error(`Failed to reload Lua: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/stats', async (req, res) => {
  try {
    ...
    logger.error(`Failed to set stats: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/releasesafehouse', async (req, res) => {
  try {
    ...
    logger.error(`Failed to release safehouse: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/steam-update', async (req, res) => {
  try {
    ...
    try {
      ...
      logger.warn(`Could not verify server status before update: ${e.message}`);
    ...
    logger.info(`Starting PZ server ${operation} (branch: ${selectedBranch})...`);
    ...
    steamcmd.on('close', (code) => {
      ...
      logger.info(`SteamCMD ${operation} finished with code ${code}`);
    });
    ...
    logger.error(`Steam update failed: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/steamcmd/download', async (req, res) => {
  try {
    ...
    logger.info(`Downloading SteamCMD to ${installPath}`);
    ...
    const handleDownloadError = (err) => {
      ...
      logger.error(`SteamCMD download failed: ${err.message}`);
    };
      ...
      try {
        ...
        steamcmd.on('close', (code) => {
          if (code === 0 || code === 7) { // Code 7 is also success for steamcmd
            ...
            logger.info(`SteamCMD installed successfully to ${installPath}`);
          } else {
            io.emit('steamcmd:status', { 
              status: 'error', 
              message: `SteamCMD setup failed with code ${code}` 
            });
            logger.error(`SteamCMD first-run failed with code ${code}`);
          }
        });
        ...
        steamcmd.on('error', (error) => {
          ...
          logger.error(`SteamCMD run error: ${error.message}`);
        });
        ...
        logger.error(`SteamCMD extraction failed: ${extractError.message}`);
      }
    }
    ...
    logger.error(`SteamCMD download failed: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
$browser.Description = '${description.replace(/'/g, "''")}'
$browser.RootFolder = [System.Environment+SpecialFolder]::MyComputer
${initialPath && isValidPath(initialPath) ? `$browser.SelectedPath = '${initialPath.replace(/'/g, "''")}'` : ''}
...
`;
    ...
    powershell.on('close', (code) => {
      ...
      if (code !== 0 || errorOutput) {
        logger.warn(`Folder browser had issues: ${errorOutput}`);
      }
    ...
    powershell.on('error', (error) => {
      logger.error(`Folder browser error: ${error.message}`);
      res.status(500).json({ error: 'Failed to open folder browser' });
    });
    
  } catch (error) {
    logger.error(`Browse folder failed: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.get('/console-log', async (req, res) => {
  try {
    ...
    logger.error(`Failed to read server console log: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.get('/console-log/stream', async (req, res) => {
  try {
    ...
    logger.error(`Failed to stream server console log: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/console-log/clear', async (req, res) => {
  try {
    ...
    if (fs.existsSync(consoleLogPath)) {
      ...
      logger.info('Server console log cleared');
    }
    
    res.json({ success: true });
  } catch (error) {
    logger.error(`Failed to clear server console log: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/update-check/interval', async (req, res) => {
  try {
    ...
    logger.error(`Failed to set update check interval: ${error.message}`);
    res.status(500).json({ error: error.message });
  }
});

...
- **If you are on v1.2.15 or older, in-app updating has been impossible - not unreliable, impossible - and this release fixes it.** Every attempt staged the download correctly, swapped correctly, and was then rejected by the new version and rolled back. Re-downloading could never help, because the file being rejected is written by the version you are updating *from*. (#149)

### Fixed

**Panel updater**

- **In-app updates from v1.2.15 or older always failed, every time, on every machine.** When an update is prepared, the running panel writes a small record of what it staged, and the incoming version reads it back before starting. v1.2.16 added a new required entry to that record but kept its version number unchanged - so the record written by v1.2.15 was structurally valid, correctly produced, and unconditionally rejected by everything after it. The update rolled back and the panel returned to the old version, with no way through: the record is regenerated by the old version on every attempt, so each retry reproduced the identical failure. Nothing the user did could affect the outcome, and the panel's own advice to re-download the update was guaranteed not to help.

  Records from older versions are now recognised and accepted, and newly written ones carry a correctly incremented version number so this cannot recur. **This fix lives in the incoming version, so it applies itself: updating to this release from an affected version now works.** (#149)

- **Integrity checking of the staged interface files is preserved for every version that has it.** The first attempt at the fix above keyed the check on the record's version number, which would have silently skipped verification for v1.2.16 through v1.2.20 - the majority of installs - because those versions also write the older version number. The check now runs whenever a checksum is present, so it is skipped only for records that genuinely never had one.
...
- **When an update fails, the panel's own startup output is now saved into the update log**, so the specific reason it refused to start is recorded rather than lost. This capture is limited to the update-apply window and bounded in size, so ordinary restarts are unaffected. The launcher also now records each step of the file swap as it happens, instead of only some of them - a log that was silent on success could not be used to tell "this step worked" from "this step never ran".
